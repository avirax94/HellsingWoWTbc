...
void Player::RegenerateHealth(uint32 diff)
{
    ...
    // caso de regeneración normal (tal vez en parte en caso de combate)
    if (!IsInCombat() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
    {
    ...
    // siempre bonificación de regeneración (incluido el combate)
...
{
    // nota: esto también se puede llamar cuando el jugador está vivo
    // por ejemplo de WorldSession :: HandleMovementOpcodes
    ...
    for (UnitList::const_iterator i = stealthedUnits.begin(); i != stealthedUnits.end(); ++i)
    {
        ...
        if (hasDetected)
        {
            if (!hasAtClient)
            {
                ...
                // duración del aura de destino para la presentación del lanzador solo si el objetivo existe en el cliente lanzador
                // enviar datos al cambio de visibilidad del objetivo (agregar al cliente)
