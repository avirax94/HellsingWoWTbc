...
struct HuntersMark : public AuraScript
{
    int32 OnAuraValueCalculate(Aura* aura, Unit* /*caster*/, int32 damage) const override
    {
        if (aura->GetEffIndex() == EFFECT_INDEX_2)
        {
            ...
            if (Aura* otherAura = aura->GetHolder()->m_auras[EFFECT_INDEX_1])
                auraValue = otherAura->GetAmount(); // buscar aura a distancia AP
            ...
            if (Unit* caster = aura->GetCaster())
            {
                ...
                for (Unit::AuraList::const_iterator j = mclassScritAuras.begin(); j != mclassScritAuras.end(); ++j)
                {
                    switch ((*j)->GetModifier()->m_miscvalue)
                    {
                        ...
                        case 5239:
                        case 5240:
                            damage = std::min((*j)->GetModifier()->m_amount, auraValue); // use más bajo, para que el talento no haga OP de bajo nivel
                            break;
                    }
                }
            }
    ...
    {
        if (aura->GetEffIndex() != EFFECT_INDEX_0) // aumenta la fuerza de la desventaja en cada golpe hasta el cuarto
        {
...
struct Misdirection : public SpellScript
{
    ...
    {
        // Parche 2.3.0 (13-11-2007):
        // Desorientación: si un cazador intenta usar esta habilidad en un objetivo que ya tiene una Desdirección activa, el hechizo no se aplicará debido a que ya está en efecto un hechizo más poderoso.
